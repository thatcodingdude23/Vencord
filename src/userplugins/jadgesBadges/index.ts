    ...
    await Promise.all(Object.entries(data).map(async ([userId, badges]) => {
        ...
        output[userId] = await Promise.all(badges.map(async badge => {
            ...
            try {
                ...
                console.error(`[JadgesBadges v9] Failed to prepare ${badge.badge}:`, error);
                return badge;
            }
        }));
    }));
...
async function refreshBadges(): Promise<void> {
    ...
    try {
        ...
        console.warn(`[JadgesBadges v9] Loaded ${count} badge(s) with native hover and raw images.`);
    } catch (error) {
        console.error("[JadgesBadges v9] Failed to refresh badges:", error);
    }
}
...
export default definePlugin({
    ...
    async start() {
        console.warn("[JadgesBadges v9] Starting native-wrapper/raw-image build.");
